...
var actorChars = {
  ...
  "v": Lava, "a": Lava, "V": Lava, "A": Lava,
  "e": Enemy, "E": Enemy, 
  "b": BouncerEnemy, "B": BouncerEnemy,
  "G": Snitch
};
...
function Coin(pos) {
  ...
  this.size = new Vector(1.15, 1.25);
...
function BouncerEnemy(pos, ch) {
  this.pos = pos;
  this.size = new Vector (1.2, 1.4);
  if (ch == "b"){
	this.speed = new Vector(-2, 0);
  } else if (ch == "B") {
...
BouncerEnemy.prototype.type = "bouncerEnemy";
...
Lava.prototype.act = function(step, level) { //animate dripping lava
...
Snitch.prototype.act = function(step) {
	var wobbleSpeed = 16, wobbleDist = 1;
...
BouncerEnemy.prototype.act = function(step, level) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "bouncerEnemy" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
    ...
    if (!this.actors.some(function(actor) {
      ...
      this.status = "won";
      this.finishDelay = 1; // for some reason this is screwing up.
    }
  } else if (type == "snitch") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
...
var arrowCodes = {37: "left", 32: "up", 39: "right"}; // Left, Space, Right
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
		...
		document.write('<img src="http://icdn4.digitaltrends.com/image/a_winner_is_you_1024-1024x768.jpg">');
        console.log("You win!");
    });
  }
  startLevel(0);
